...
def load_dataset(T=4, nb_flow=2, len_closeness=None, len_period=None, len_trend=None,
    ...
    if os.path.exists(fname):
        ...
        X_train, Y_train, X_test, Y_test, mmn, external_dim, timestamp_train, timestamp_test = \
            load_dataset(len_closeness=len_closeness, len_period=len_period, len_trend=len_trend,
                         len_test=len_test, meta_data=True, meteorol_data=False, holiday_data=True)
        for i in range(3):
            X_train[i] = np.transpose(X_train[i],(0, 3, 1, 2))
            X_test[i] = np.transpose(X_test[i], (0, 3, 1, 2))
        Y_train = np.transpose(Y_train, (0, 3, 1, 2))
